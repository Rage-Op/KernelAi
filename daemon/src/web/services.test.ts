...
test('services: lists exactly the three known background services', async () => {
  const svcs = await listServices();
  assert.deepEqual(
    svcs.map((s) => s.name).sort(),
    ['browser', 'lmstudio', 'stray-daemons'],
  );
  for (const s of svcs) {
...
test('services: action is hard-allowlisted — unknown name / non-stop action are refused (no process touched)', async () => {
  ...
  assert.match(await runServiceAction('lmstudio', 'restart'), /unsupported action/);
