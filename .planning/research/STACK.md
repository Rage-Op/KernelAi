...
**Domain:** Persistent local AI orchestrator for macOS (long-lived TS/Node daemon + native Swift/SwiftUI menubar "face")
**Researched:** 2026-06-22
**Confidence:** HIGH (versions verified against npm registry + official docs as of June 2026)

> Scope note: the high-level stack is **pinned** by `docs/KERNEL_MASTER_BUILD_PROMPT.md` §2. This document makes those pins implementation-ready: exact versions, invocation details, and explicit "do NOT use X" notes. Target hardware is a **MacBook Pro 16" / M2 Pro / 16GB**, and the 16GB ceiling drives several choices flagged below.
...
Source of truth: `anthropics/skills` repo model catalog (live), cross-checked with the local GSD model catalog. **Use versioned IDs, never aliases**, for a daemon you do not want silently re-pointed.
...
> Do NOT use `claude-opus-4-1`/`4-0`, `claude-sonnet-4-0`, `claude-haiku-3`, or any `-latest` alias — the first group is deprecated/retiring (4-1 retires 2026-08-05), aliases drift. `claude-fable-5` exists and is the top tier but is overkill/pricier ($10/$50 MTok) for KERNEL's loop; default to Opus 4.8.
...
**16GB call-out:** keep exactly **one** local model loaded. Lean on Ollama's idle-unload (`OLLAMA_KEEP_ALIVE` default 5m). The local 7B is the "always-on cheap helper" (§6) — but on 16GB "always-on" means "reloads on demand"; accept the cold-start (~1–3s) rather than pinning with `keep_alive: -1`, which would permanently hold ~6GB and starve the browser/Metal.
...
fnm install 24 && fnm use 24          # or nvm
...
npm install @anthropic-ai/sdk@0.105.0 \
            ...
            playwright@1.61.0 \
            pino@10.3.1 zod@4.4.3 yaml gray-matter execa simple-git \
            plaid@42.2.0 better-sqlite3-multiple-ciphers@12.11.1

# Dev deps
npm install -D typescript@5.9 tsx pino-pretty @types/node
...
sh ./models/download-ggml-model.sh base.en       # or small.en
...
peekaboo permissions request-event-synthesizing    # needed for coord clicks + keyboard
```
...
- **Permissions (System Settings → Privacy & Security):**
  - **Screen Recording** — required (capture + multi-app automation).
  - **Accessibility** — required for reliable focus/menu/dialog interaction.
  - **Event synthesizing** — required for coordinate clicks + keyboard input (`peekaboo permissions request-event-synthesizing`).
  - Mail/Contacts/Calendar prompt their own consent on first access.
...
**If local-brain mode is toggled on (§6, privacy):**
...
**If Metal particle work risks the Phase 2 deadline:**
...
**If `small.en` latency hurts the live-conversation feel:**
- Fall back to `base.en`; rely on the cloud brain's quality to compensate for minor transcription errors.

**If Opus 4.8 cost is a concern for high-frequency loop calls:**
...
---
*Stack research for: persistent local AI orchestrator on macOS (M2 Pro / 16GB)*
*Researched: 2026-06-22*
